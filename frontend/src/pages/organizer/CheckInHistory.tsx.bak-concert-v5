...
const resultClass: Record<string, string> = {
    ...
    NOT_FOUND: 'bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-white/10',
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-50">Lịch sử check-in</h1>
                ...
                <div className="flex gap-2">
                    <select
                        ...
                        className="border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm bg-white dark:bg-slate-900/90 focus:outline-primary"
                    >
            ...
            <Card className="border-slate-200 dark:border-white/10 rounded-2xl shadow-none overflow-hidden">
                <CardHeader className="border-b border-slate-100 dark:border-white/10">
                    <CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2"><Search size={18} /> Nhật ký quét vé</CardTitle>
                </CardHeader>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/10">
                                {isLoading ? (
                                    ...
                                    <tr key={log._id} className="hover:bg-slate-50/70">
                                        <td className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">{formatDateTime(log.scanned_at)}</td>
                                        <td className="px-6 py-4">
                                            <div className="font-semibold text-slate-800 dark:text-slate-100">{log.show_id?.name || 'Không xác định'}</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400">{log.event_id?.name || ''}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="font-semibold text-slate-700 dark:text-slate-200">{log.staff_id ? `${log.staff_id.first_name || ''} ${log.staff_id.last_name || ''}` : 'N/A'}</div>
                                        ...
                                        <td className="px-6 py-4"><span className="px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200 text-xs font-bold">{log.mode}</span></td>
