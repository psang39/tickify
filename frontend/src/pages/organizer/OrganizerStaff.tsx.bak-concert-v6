    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-50 dark:text-slate-50 flex items-center gap-3"><UserCog className="text-primary" /> Quản lý staff scanner</h1>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <Card className="xl:col-span-2 border-white/10 dark:border-white/10 rounded-2xl shadow-none overflow-hidden">
                    <CardHeader className="border-b border-slate-100 dark:border-white/10 flex flex-row items-center justify-between">
                        <CardTitle className="text-base font-bold text-slate-100 dark:text-slate-100 flex items-center gap-2"><Users size={18} /> Danh sách nhân viên</CardTitle>
                    </CardHeader>
                                    ...
                                    {isLoading ? (
                                        ...
                                        <tr key={staff._id} className={`hover:bg-slate-950/70/70 ${selectedStaffId === staff._id ? 'bg-primary/5' : ''}`}>
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-slate-100 dark:text-slate-100">{staff.last_name} {staff.first_name}</div>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800/80 text-slate-200 dark:text-slate-200 text-xs font-bold">{staff.assigned_show_ids?.length || 0} show</span>
                                            </td>
                ...
                <Card className="border-white/10 dark:border-white/10 rounded-2xl shadow-none h-fit">
                    <CardHeader className="border-b border-slate-100 dark:border-white/10">
                        <CardTitle className="text-base font-bold text-slate-100 dark:text-slate-100 flex items-center gap-2"><CalendarPlus size={18} /> Phân công show</CardTitle>
                    </CardHeader>
                    <CardContent className="p-5 space-y-4">
                        <div>
                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Nhân viên</label>
                            <select value={selectedStaffId} onChange={(e) => setSelectedStaffId(e.target.value)} className="mt-1 w-full border border-white/10 dark:border-white/10 rounded-xl px-3 py-2 text-sm bg-white dark:bg-slate-900/90 focus:outline-primary">
                        ...
                        <div>
                            ...
                            <select value={selectedEventId} onChange={(e) => { setSelectedEventId(e.target.value); setSelectedShowId(''); }} className="mt-1 w-full border border-white/10 dark:border-white/10 rounded-xl px-3 py-2 text-sm bg-white dark:bg-slate-900/90 focus:outline-primary">
                        ...
                        <div>
                            ...
                            <select value={selectedShowId} onChange={(e) => setSelectedShowId(e.target.value)} disabled={!selectedEventId} className="mt-1 w-full border border-white/10 dark:border-white/10 rounded-xl px-3 py-2 text-sm bg-white dark:bg-slate-900/90 focus:outline-primary disabled:bg-slate-950/70">
                                    ...
                                    {(selectedStaff.assigned_show_ids || []).length === 0 ? (
                                        ...
                                        return (
                                            <div key={showId} className="flex items-center justify-between gap-2 border border-slate-100 dark:border-white/10 rounded-xl p-2">
                                                <div className="text-sm font-medium text-slate-200 dark:text-slate-200 truncate">{showName}</div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <Card className="w-full max-w-md rounded-2xl shadow-xl">
                        <CardHeader><CardTitle className="text-lg font-bold text-slate-100 dark:text-slate-100">Cấp tài khoản Staff mới</CardTitle></CardHeader>
                        <CardContent>
                            <form onSubmit={handleCreateStaff} className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <input required placeholder="Họ" value={formData.last_name} onChange={e => setFormData({ ...formData, last_name: e.target.value })} className="border border-white/10 dark:border-white/10 px-3 py-2 rounded-xl text-sm focus:outline-primary" />
                                    <input required placeholder="Tên" value={formData.first_name} onChange={e => setFormData({ ...formData, first_name: e.target.value })} className="border border-white/10 dark:border-white/10 px-3 py-2 rounded-xl text-sm focus:outline-primary" />
                                </div>
                                <input required type="email" placeholder="Email đăng nhập" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full border border-white/10 dark:border-white/10 px-3 py-2 rounded-xl text-sm focus:outline-primary" />
                                <input required type="tel" placeholder="Số điện thoại" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="w-full border border-white/10 dark:border-white/10 px-3 py-2 rounded-xl text-sm focus:outline-primary" />
                                <input required type="password" placeholder="Mật khẩu ban đầu" value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} className="w-full border border-white/10 dark:border-white/10 px-3 py-2 rounded-xl text-sm focus:outline-primary" />
