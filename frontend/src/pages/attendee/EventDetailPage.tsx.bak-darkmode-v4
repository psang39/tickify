...
export default function EventDetailPage() {
    ...
    if (isLoading) {
        return <div className="mx-auto max-w-7xl px-6 py-14"><div className="h-96 animate-pulse rounded-3xl bg-slate-100" /></div>;
    }
    ...
    if (!event) {
        return (
            <div className="mx-auto max-w-7xl px-6 py-20 text-center">
                ...
                <p className="mt-2 text-sm text-slate-500">Sự kiện có thể đã bị ẩn hoặc không còn tồn tại.</p>
    ...
    return (
        <div className="bg-white font-sans text-slate-900">
            <section className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
                <button onClick={() => navigate(-1)} className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-[#FF0082]">
            ...
            <section className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 pb-8 lg:grid-cols-3 lg:px-8">
                <main className="lg:col-span-2">
                    <div className="rounded-3xl border border-slate-200 bg-white p-6">
                        <h2 className="text-xl font-bold text-slate-900">Thông tin sự kiện</h2>
                        <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div className="rounded-2xl bg-slate-50 p-4">
                                <p className="text-xs font-bold uppercase text-slate-400">Thời gian</p>
                                <p className="mt-2 flex items-center gap-2 text-sm font-semibold text-slate-700"><CalendarDays size={16} />{formatDate(event.start_date)} - {formatDate(event.end_date)}</p>
                            </div>
                            <div className="rounded-2xl bg-slate-50 p-4">
                        ...
                        <div className="mt-6 border-t border-slate-100 pt-6">
                            <h3 className="text-sm font-bold text-slate-900">Mô tả</h3>
                            <p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">{event.description || 'Thông tin chi tiết sự kiện sẽ được cập nhật trong thời gian tới.'}</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <h2 className="text-xl font-bold text-slate-900">Danh sách show</h2>
                                <p className="mt-1 text-sm text-slate-500">Chọn một show để xem sơ đồ ghế và đặt vé.</p>
                            </div>
                            {totalPages > 1 && (
                                <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                                    Trang {page}/{totalPages}
                                    <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="rounded-full border border-slate-200 p-2 disabled:opacity-40"><ChevronLeft size={16} /></button>
                                    <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} className="rounded-full border border-slate-200 p-2 disabled:opacity-40"><ChevronRight size={16} /></button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                            {isShowsLoading ? (
                                [1, 2, 3, 4].map((item) => <div key={item} className="h-36 animate-pulse rounded-2xl bg-slate-100" />)
                            ) : shows.length === 0 ? (
                                <div className="rounded-2xl bg-slate-50 p-8 text-center text-sm font-medium text-slate-400 md:col-span-2">Hiện chưa có show nào đang mở bán công khai.</div>
                            ) : shows.map((show: any) => {
                                const venue = show.venue_id || show.venue_info;
                                const isBookable = show.status === 'published';
                                return (
                                    <article key={show._id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                                        <div className="flex">
                                            ...
                                            <div className="flex flex-1 flex-col gap-2 p-4">
                                                <h3 className="text-sm font-bold text-slate-900">{show.name}</h3>
                                                <p className="flex items-center gap-2 text-xs font-medium text-slate-500"><Clock size={14} />{formatTime(show.start_time)}</p>
                                                <p className="flex items-center gap-2 text-xs font-medium text-slate-500"><MapPin size={14} />{venue?.name || 'Địa điểm đang cập nhật'}{venue?.city ? `, ${venue.city}` : ''}</p>
                                                <button
                                                    disabled={!isBookable}
                ...
                <aside className="space-y-5">
                    <div className="rounded-3xl border border-slate-200 bg-white p-5">
                        <h3 className="text-base font-bold text-slate-900">Poster sự kiện</h3>
                        <div className="mt-4 aspect-[3/4] overflow-hidden rounded-2xl bg-slate-100">
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-white p-5">
                        <h3 className="text-base font-bold text-slate-900">Lưu ý đặt vé</h3>
                        <p className="mt-3 text-sm leading-6 text-slate-500">Bạn nên kiểm tra kỹ thời gian, địa điểm và trạng thái mở bán của từng show trước khi đặt vé. Vé sau khi thanh toán sẽ được lưu trong lịch sử đặt vé.</p>
                    </div>
                </aside>
            </section>
            ...
            <section className="mx-auto max-w-7xl px-6 pb-14 lg:px-8">
                <div className="mb-5 flex items-center justify-between">
                    ...
                    <Link to="/search" className="text-sm font-semibold text-slate-500 hover:text-[#FF0082]">Xem thêm</Link>
                </div>
