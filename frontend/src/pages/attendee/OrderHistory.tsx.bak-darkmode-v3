    ...
    return (
        <div className="w-full max-w-4xl animate-in fade-in duration-300">
            ...
            <header className="mb-10">
                ...
                <p className="text-slate-500 text-sm">Dưới đây là toàn bộ lịch sử đơn đặt vé và trạng thái thanh toán của bạn.</p>
            </header>
...
function OrderCard({
    ...
    return (
        <article
            onClick={onClick}
            className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-none transition-all hover:border-primary/40 hover:bg-slate-50/40 cursor-pointer"
        >
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="min-w-0">
                            <h4 className="font-black text-base text-slate-800 leading-snug break-words">{eventName}</h4>
                            <p className="text-xs text-slate-500 font-semibold mt-1 break-words">{showName}</p>
                        </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500">
                            ...
                            <span className="rounded-full bg-slate-100 px-3 py-1.5">{statusMeta.description}</span>
                        </div>

                        <div className="flex items-center justify-between sm:justify-end gap-3">
                            <span className="font-black text-base text-slate-900">{formatCurrency(order.total_price)}</span>
