    ...
    switch ((status || '').toLowerCase()) {
        ...
        default:
            return {
                ...
                className: 'bg-slate-100 dark:bg-slate-800/80 text-slate-600 border-white/10 dark:border-white/10 dark:border-slate-500/30 dark:bg-slate-800/70 dark:text-slate-300'
            };
    }
}
...
function InfoPill({ icon, label, value, className = '' }: { icon: ReactNode; label: string; value: string; className?: string }) {
    return (
        <div className={`rounded-xl border border-slate-100 dark:border-white/10 bg-slate-950/70/80 px-3 py-2.5 ${className}`}>
