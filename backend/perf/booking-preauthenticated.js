...
import { admission, disjointSeatForVu, hold, manifest, orderId, preissuedTokenForVu, seatMap } from './lib/flow.js';

// Control workload: skips only /auth/login. Each VU uses its own seed-issued,
// short-lived token while retaining admission, concurrent layout/status loading,
// and hold exactly as in booking-baseline.js.
...
export default function () {
    const token = preissuedTokenForVu(__VU);
    ...
    const response = hold(
        manifest.booking.showId,
        token,
        admitted.checkoutToken,
        disjointSeatForVu(__VU),
        manifest.booking.ticketTypeId,
    );
