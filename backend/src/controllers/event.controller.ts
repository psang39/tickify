...
import { deleteLocalUploadedImage, getUploadedEventImageUrl } from '../utils/eventImageUpload'; import { computeShowAvailability } from '../utils/showAvailability';
...
export const searchEventsPublic = async (req: Request, res: Response) => {
  try {
    ...
    if (city && city.trim() !== '' && city !== 'undefined' && city !== 'all') {
      const matchingVenues = await Venue.find({ city: { $regex: `^${city.trim()}import Event from "../models/event.model";
import { Request, Response } from "express";
import mongoose from "mongoose";
import Show from "../models/show.model";
import Order from "../models/order.model";
