...
export const searchEventsPublic = async (req: Request, res: Response) => {
    try {
        ...
        const events = await Event.find(findQuery)
            .select('_id name genre artists poster_url banner_url start_date end_date created_at status')
            .sort(sortOption)
            .limit(safeLimit)
            .lean();
        ...
        relatedShows.forEach((show: any) => {
            const key = String(show.event_id || show.event);
